...
if(null !== filter_input(INPUT_POST, 'perspective_edit_submit')) {
    ...
    if($form_valid) {
        $sql = "update perspective set customer_id = $customer_id, date = '$date', date_minus = '$date_minus', date_plus = '$date_plus', "
                . "film_id = $film_id, film_variation_id = $film_variation_id, expenses = $expenses, film_width = $film_width, film_length = $film_length, "
                . "film_weight = $film_weight, film_price = $film_price, film_currency = '$film_currency', probability = $probability where id = $id";
...
$sql = "select customer_id, date, date_minus, date_plus, film_id, film_variation_id, expenses, "
        . "film_width, film_length, film_weight, film_price, film_currency, probability "
        . "from perspective where id = $id";
...
if($row = $fetcher->Fetch()) {
    ...
    $expenses = filter_input(INPUT_POST, 'expenses');
    if(empty($expenses)) {
        $expenses = $row['expenses'];
    }
    
    $film_width = filter_input(INPUT_POST, 'film_width');
    if(empty($film_width)) {
        $film_width = $row['film_width'];
    }
    
    $film_length = filter_input(INPUT_POST, 'film_length');
    if(empty($film_length)) {
        $film_length = $row['film_length'];
    }
    
    $film_weight = filter_input(INPUT_POST, 'film_weight');
    if(empty($film_weight)) {
        $film_weight = $row['film_weight'];
    }
    
    $film_price = filter_input(INPUT_POST, 'film_price');
    if(empty($film_price)) {
        $film_price = $row['film_price'];
    }
    
    $film_currency = filter_input(INPUT_POST, 'film_currency');
    if(empty($film_currency)) {
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12 col-md-6">
                    <h1>Редактирование планируемого действия</h1>
                    <form method="post">
                        ...
                        <div class="row">
                            <div class="col-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date_minus" value="<?= $date_minus ?>" class="form-control" />
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="form-group">
                            ...
                            <div class="col-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date_plus" value="<?= $date_plus ?>" class="form-control" />
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="form-group">
                                    <label for="expenses">Затраты</label>
                                    <input type="text" name="expenses" value="<?= $expenses ?>" class="form-control int-only" />
                                </div>
                            </div>
                            <div class="col-4"></div>
                            <div class="col-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="probability" value="<?= $probability ?>" class="form-control int-only" />
                                </div>
                            </div>
                        </div>
                        <div class="row mt-3">
                            ...
                            <div class="col-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="film_width" value="<?= $film_width ?>" class="form-control int-only" />
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="form-group">
                                    <label for="film_length">Длина плёнки</label>
                                    <input type="text" name="film_length" value="<?= $film_length ?>" class="form-control int-only" />
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="form-group">
                                    <label for="film_weight">Вес плёнки</label>
                                    <input type="text" name="film_weight" value="<?= $film_weight ?>" class="form-control int-only" />
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="film_price" value="<?= $film_price ?>" class="form-control int-only" />
                        ...
                        <div class="mt-4">
                            <button type="submit" class="btn btn-dark w-25" id="perspective_edit_submit" name="perspective_edit_submit">Редактировать</button>
                        </div>
                    </form>
                </div>
            </div>
